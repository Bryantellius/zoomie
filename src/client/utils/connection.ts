...
class SocketConnection {
  ...
  setNavigatorToStream = () => {
    this.getVideoAudioStream().then((stream: any) => {
      if (stream) {
        this.streaming = true;
  ...
  getVideoAudioStream = (video: boolean = true, audio: boolean = true) => {
    ...
    // @ts-ignore
    const myNavigator = navigator.mediaDevices as any;
    const nowNavigator =
  ...
  connectToNewUser(userData: any, stream: any) {
  ...
  listenToEndStream = (stream: any, status: MediaStatus) => {
  ...
  toggleVideoTrack = (status: MediaStatus) => {
    const myVideo: any = this.getMyVideo();
    // @ts-ignore
...
const initializePeerConnection = () => {
  return new Peer("", {
    host: config.peerjsEndpoint,
    secure: true,
  });
};

const initializeSocketConnection = () => {
  return (openSocket as any).connect(config.websocket, {
    secure: true,
    reconnection: true,
